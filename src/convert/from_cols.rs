...
mod tests {
    ...
    fn test_from_cols() {
        ...
        let matrix = Matrix::from_cols(rows);
    ...
    fn test_try_from_cols() {
...

        {
            ...
            let matrix = Matrix::try_from_cols(rows).unwrap();
            testkit::assert_strict_eq(&matrix, &expected);
...

        {
            ...
            assert!(Matrix::try_from_cols(rows).is_ok());
...

        {
            ...
            let error = Matrix::try_from_cols(rows).unwrap_err();
    ...
    fn test_from_row_iter() {
        ...
        let matrix = Matrix::from_col_iter(rows);
        testkit::assert_strict_eq(&matrix, &expected);
    }
