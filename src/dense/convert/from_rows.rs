...
mod tests {
    ...
    fn test_from_rows() {
        let expected = {
            ...
            Matrix::<u8, RowMajor> { layout, data }
        };

        dispatch_unary! {{
            let seq: [[u8; 3]; 2] = [[1, 2, 3], [4, 5, 6]];
            let output = Matrix::<u8, O>::from_rows(seq);
    ...
    fn test_try_from_rows() {
        ...
        dispatch_unary! {{
            ...
            let output = Matrix::<u8, O>::try_from_rows(seq).unwrap();
            assert_eq!(output, expected);
    ...
    fn test_from_row_iter() {
        ...
        dispatch_unary! {{
            ...
            let output = Matrix::<u8, O>::from_row_iter(iter);
            assert_eq!(output, expected);
        }}
    }
    ...
    fn test_from_row_iter_fails_row_major() {
        ...
        Matrix::<u8, RowMajor>::from_row_iter(iter);
    }
    ...
    fn test_from_row_iter_fails_col_major() {
        let iter = [vec![1, 2, 3], vec![4, 5]];
        Matrix::<u8, ColMajor>::from_row_iter(iter);
    }
}
